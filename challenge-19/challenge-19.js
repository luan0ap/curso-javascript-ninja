( function () {
  ...
  */
  ...
  function hasClass ( markup, cssClass ) {
    var regex = new RegExp( 'class=["\'](?:[\w\s]+)?' + cssClass + '(?:[\w\s]+)?["\']\s+')
    return regex.test( markup )
  }
  
  ;[ 'container', 'text', 'date', 'exerpt', 'main' ].forEach( function( class ) {
    console.log( hasClass( markup, class ) + ' para a classe ' + class )
  } )
